...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((acc,ele)=>{return acc+=ele.purchasePrice;},0);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((acc,ele)=>acc+=1,0);
};
...
const reversedString = (str) => {
  // Solution code here...
  const arr = str.split(" ");
  const arr2 = arr.map(element=>element.split("")
      .reduce((acc,ele,idx)=>{
          if(idx == 0){
            acc.push(ele);
          }else{
  ...
  const str1 = arr2[0].join("");
  const str2 = arr2[1].join("");
  const str3 = str2 + ' ' + str1;    
  return str3;
};
